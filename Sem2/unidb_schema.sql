...
CREATE TABLE planned_activities (
 activity_id VARCHAR(10) NOT NULL,
 instance_id VARCHAR(10) NOT NULL,
 planned_hours NUMERIC(6) NOT NULL
);

ALTER TABLE planned_activities ADD CONSTRAINT PK_planned_activities PRIMARY KEY (activity_id,instance_id);

CREATE UNIQUE INDEX pk_planned_activities ON planned_activities (activity_id,instance_id);

CREATE INDEX idx_planned_activities_instance_activity ON planned_activities (activity_id,instance_id);
...
ALTER TABLE allocation ADD CONSTRAINT PK_allocation PRIMARY KEY (allocation_id,teacher_id,activity_id,instance_id);

CREATE UNIQUE INDEX pk_allocation ON allocation (allocation_id,teacher_id,activity_id,instance_id);

CREATE INDEX idx_allocation_instance_activity ON allocation (teacher_id,instance_id,activity_id);
